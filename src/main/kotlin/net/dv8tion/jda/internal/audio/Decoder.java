 ...
 */
/**
 * PAWA PATCH — verbatim copy of JDA v6.4.2 Decoder except decodeFromOpus is
 * synchronized with a null guard. Stock JDA races decodeFromOpus (receive
 * thread) against close() (AudioConnection.shutdown / removeUserSSRC), passing
 * a null/freed pointer to native opus_decode -> daily SIGSEGV in production.
 * Mirrors upstream's own encoder-side fix (JDA issue #2880). This class
 * replaces JDA's copy in the shaded jar (see shade filter in pom.xml).
 * Guarded by DecoderRaceTest — delete this file when its canary test fails
 * (that means upstream fixed the bug).
 */
public class Decoder {
    ...
    public synchronized short[] decodeFromOpus(AudioPacket decryptedPacket) {
        if (opusDecoder == null) {
            return null; // decoder was closed concurrently; drop the packet (PAWA PATCH)
        }
        int result;
